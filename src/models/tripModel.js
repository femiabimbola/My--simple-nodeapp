...
class TripModel extends IndexModel {
  ...
  async createTripQuery({ bus_id, origin, destination, trip_date, fare }) {
    try {
      const {
        rows
      } = await this.insert(
  ...
  async cancelTrip(tripId) {
    try {
      ...
      const { rows } = await this.update("status=$1", "id=$2", [
        "cancelled",
        tripId
      ]);
      return rows[0];
  ...
  async checkBusAvailability(bus_id) {
    try {
      const { rows } = await this.selectWhere("*", "bus_id=$1 and status=$2", [
        bus_id,
        "active"
      ]);
      return rows[0];
